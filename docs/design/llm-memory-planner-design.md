...
- 应用 vLLM `v0.25.0`、SGLang `v0.5.15` 的版本化默认值；
...
推理框架和框架版本也是整份 Deployment Config 的顶层公共配置。普通部署和 PD 部署中的所有部署组必须使用同一个框架及同一个固定版本；MVP 为 vLLM `v0.25.0` 或 SGLang `v0.5.15`。

vLLM `v0.25.0` 的 framework adapter、默认值、rank/group、组件放置和能力矩阵必须以该 release 的固定 tag 源码建立。当前源码基线见 [vLLM v0.25.0 并行策略源码审计](../research/2026-07-13-vllm-v0.25.0-parallelism-source-audit.md)；现有 `v0.24.0` 审计只保留为升级差异的历史基线，不得作为当前 adapter 的直接实现依据。exact-tag 源码审计确认实现语义，但不替代目标模型、attention backend、量化和拓扑组合的运行验证。
