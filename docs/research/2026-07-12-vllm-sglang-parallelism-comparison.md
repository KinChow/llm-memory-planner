...
> 状态：设计输入，非最终方案
>
> 版本状态：历史对比基线。需求 v2.0 已将当前 vLLM 基线升级到 `v0.25.0`；涉及 vLLM 的结论必须等待 `v0.25.0` exact-tag 重审后更新
>
> 对比日期：2026-07-12
>
> 版本基线：vLLM `v0.24.0`（`ee0da84ab9e04ac7610e28580af62c365e898389`）；SGLang `v0.5.15`（`f63458b5beaceabbd9d749b9fc956370e1b649e6`）
>
> 证据来源：[vLLM v0.24.0 并行策略源码审计](./2026-07-12-vllm-v0.24.0-parallelism-source-audit.md)；[SGLang v0.5.15 并行策略源码审计](./2026-07-12-sglang-v0.5.15-parallelism-source-audit.md)
>
> 目的：明确两个框架哪些概念可以投影到共同的显存账本，哪些语义必须由框架版本适配器独立解析。本文件不定义最终配置 schema。

本文仍保留 vLLM `v0.24.0` 与 SGLang `v0.5.15` 的原始对比，以便识别升级差异；它不代表当前 `v0.25.0` 与 SGLang `v0.5.15` 的最终对比结论。
...
| 主题 | vLLM `v0.24.0` | SGLang `v0.5.15` | 显存规划含义 |
|---|---|---|---|
...
| Prefill context parallel | PCP 增加 worker，且该版本支持状态有限 | Attention CP 在 TP world 内进一步分解 attention 轴 | 同名 CP 对设备数、权重和 workspace 的影响相反或不同 |
...
目标版本中模型并行初始化构造 TP、DCP、PCP、PP、DP、EP，必要时还构造 EPLB group。rank 布局在源码中以 External-DP、DP、PP、PCP、TP 坐标组织；其中 DCP 是对 TP ranks 的子分组，不增加 worker。

关键源码锚点：

- `vllm/config/parallel.py:791-802`：基础 world size 及 external launcher 下的 DP 处理；
- `vllm/distributed/parallel_state.py:1694-1935`：rank 坐标与各 process group 构造；
- `vllm/distributed/parallel_state.py:1870-1919`：MoE EP/EPLB group 跨 DP、PCP、TP 的形成。
...
vLLM `v0.24.0` 不把用户指定的 `ep_size` 作为额外独立维度。MoE expert domain 从现有 DP、PCP、TP ranks 派生。`enable_expert_parallel` 决定 routed experts 是在该 domain 内以 tensor-parallel 方式切分，还是以 expert-parallel 方式分配。
...
vLLM Prefill Context Parallel（PCP）是基础 world size 的乘数，会增加 worker，并影响 Prefill token/context 分配、KV 布局和 MoE group。目标版本虽然已有配置与 process-group 骨架，但内置 attention backend 的正式支持边界有限，不能因为 CLI 参数存在就默认可运行。
...
其中每一项的“owned”都来自 topology 和 cache strategy，而不是原始并行参数的简单除法。
...
1. **vLLM PCP 的 MVP 边界**：目标版本统一判 `unsupported/experimental`，还是只允许明确验证过的模型/backend 组合。
